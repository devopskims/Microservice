```shell
export AWS_PROFILE=won # 자신의 AWS 프로필 이름을 정해주세요.
...
serverless deploy --aws-profile won --region ap-northeast-2
...
serverless remove --aws-profile won --region ap-northeast-2 --stage dev --verbose
...
serverless deploy --aws-profile FILL_ME_IN --region ap-northeast-2

# Deploying aws-node-sqs-worker to stage dev (ap-northeast-2)

# ✔ Service deployed to stack aws-node-sqs-worker-dev (34s)

# endpoint: POST - https://rr298yy7hk.execute-api.ap-northeast-2.amazonaws.com/producer_consumer
# functions:
#   producer_consumer: aws-node-sqs-worker-dev-producer_consumer (22 kB)
```


다음과 같이 데이터가 잘 들어간 것을 확인할 수 있습니다.

> `curl -w "@curl-format.txt"`
>
> 관련 정보: https://stackoverflow.com/questions/18215389/how-do-i-measure-request-and-response-times-at-once-using-curl
...
{"message":"Message accepted! Result: 2"}
     time_namelookup:  0.017786s
        time_connect:  0.025264s
     time_appconnect:  0.065908s
    time_pretransfer:  0.066011s
       time_redirect:  0.000000s
  time_starttransfer:  0.066037s
                     ----------
          time_total:  5.363847s
